...
class InteractiveDxSession:
    ...
    def step(self, physician_input: str) -> None:
        ...
        if last_action.action_type == "test":
            cost = self._orchestrator._estimate_cost(last_action.content)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "InteractiveDxSession":
        ...
        if data.get("case_state"):
            session.case_state = CaseState(**data["case_state"])
            # Rehydrate last_actions to Pydantic Action objects
            last_actions = data["case_state"].get("last_actions", [])
            session.case_state.last_actions = [
                Action(**a) if not isinstance(a, Action) else a
